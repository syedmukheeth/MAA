 ...
 * per line — which is why the hero headline gives each line its own span.
 *
 * `custom` is the line index.
 */
export const revealMask = {
  hidden: { y: "110%" },
  visible: (i = 0) => ({
    y: "0%",
    transition: { duration: DUR.hero, ease: EASE.out, delay: 0.15 + i * 0.1 },
  }),
};
